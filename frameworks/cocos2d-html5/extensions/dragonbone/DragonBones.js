...
(function (dragonBones) {
    ...
    (function (animation) {
        ...
        var Animation = (function () {
            ...
            Animation.prototype.getCurrentAnimationName = Animation.prototype.getLastAnimationName;
    ...
    (function (objects) {
        ...
        var DataParser = (function () {
            ...
            DataParser.parseTextureAtlasData = function (rawData, scale) {
                ...
                var subTextureList = rawData.SubTexture;
                for (var index in subTextureList) {
                    if (!subTextureList.hasOwnProperty(index)) continue;
                    var subTextureObject = subTextureList[index];
                    var subTextureName = subTextureObject.name;
                    var subTextureData = new geom.Rectangle(Number(subTextureObject.x) / scale, Number(subTextureObject.y) / scale, Number(subTextureObject.width) / scale, Number(subTextureObject.height) / scale);
            ...
            DataParser.parseSkeletonData = function (rawData) {
                ...
                var frameRate = Number(rawData.frameRate);
                var data = new SkeletonData();
                data.name = rawData.name;

                var armatureObjectList = rawData.armature;
            ...
            DataParser.parseArmatureData = function (armatureObject, data, frameRate) {
                ...
                var boneObjectList = armatureObject.bone;
                ...
                var skinObjectList = armatureObject.skin;
                ...
                var animationObjectList = armatureObject.animation;
            ...
            DataParser.parseBoneData = function (boneObject) {
                ...
                boneData.length = Number(boneObject.length) || 0;
                var scaleMode = Number(boneObject.scaleMode);
                if (!isNaN(scaleMode) && scaleMode) {
                    boneData.scaleMode = scaleMode;
                }
                var inheritRotation = boneObject.fixedRotation;
                if (inheritRotation) {
                    boneData.fixedRotation = inheritRotation;
                }

                DataParser.parseTransform(boneObject.transform, boneData.global);
            ...
            DataParser.parseSkinData = function (skinObject, data) {
                ...
                var slotObjectList = skinObject.slot;
            ...
            DataParser.parseSlotData = function (slotObject, data) {
                ...
                slotData.zOrder = Number(slotObject.z);
                ...
                var displayObjectList = slotObject.display;
            ...
            DataParser.parseDisplayData = function (displayObject, data) {
                ...
                DataParser.parseTransform(displayObject.transform, displayData.transform, displayData.pivot);
            ...
            DataParser.parseAnimationData = function (animationObject, armatureData, frameRate) {
                ...
                animationData.loop = Number(animationObject.loop) || 0;
                animationData.fadeInTime = Number(animationObject.fadeInTime);
                animationData.duration = Number(animationObject.duration) / frameRate;
                animationData.scale = Number(animationObject.scale);

                if (animationObject.hasOwnProperty(utils.ConstValues.A_TWEEN_EASING)) {
                    var tweenEase = animationObject.tweenEasing;
                ...
                var timeline;
                var timelineName;
                var timelineObjectList = animationObject.timeline;
            ...
            DataParser.parseTimeline = function (timelineObject, timeline, frameParser, frameRate) {
                var position = 0;
                var frame;
                var frameObjectList = timelineObject.frame;
            ...
            DataParser.parseTransformTimeline = function (timelineObject, duration, frameRate) {
                ...
                timeline.scale = Number(timelineObject.scale);
                timeline.offset = Number(timelineObject.offset);

                return timeline;
            };

            DataParser.parseFrame = function (frameObject, frame, frameRate) {
                frame.duration = Number(frameObject.duration) / frameRate;
            ...
            DataParser.parseTransformFrame = function (frameObject, frameRate) {
                ...
                frame.visible = Number(frameObject.hide) != 1;

                if (frameObject.hasOwnProperty(utils.ConstValues.A_TWEEN_EASING)) {
                    var tweenEase = frameObject.tweenEasing;
                    if (tweenEase == undefined || tweenEase == null) {
                ...
                frame.tweenRotate = Number(frameObject.tweenRotate) || 0;
                frame.displayIndex = Number(frameObject.displayIndex) || 0;

                //
                frame.zOrder = Number(frameObject.z) || 0;

                DataParser.parseTransform(frameObject.transform, frame.global, frame.pivot);
                frame.transform.copy(frame.global);

                var colorTransformObject = frameObject.colorTransform;
                if (colorTransformObject) {
                    frame.color = new geom.ColorTransform();
                    frame.color.alphaOffset = Number(colorTransformObject.aO);
                    frame.color.redOffset = Number(colorTransformObject.rO);
                    frame.color.greenOffset = Number(colorTransformObject.gO);
                    frame.color.blueOffset = Number(colorTransformObject.bO);

                    frame.color.alphaMultiplier = Number(colorTransformObject.aM) * 0.01;
                    frame.color.redMultiplier = Number(colorTransformObject.rM) * 0.01;
                    frame.color.greenMultiplier = Number(colorTransformObject.gM) * 0.01;
                    frame.color.blueMultiplier = Number(colorTransformObject.bM) * 0.01;
                }
            ...
            DataParser.parseTransform = function (transformObject, transform, pivot) {
                ...
                if (transformObject) {
                    if (transform) {
                        transform.x = Number(transformObject.x);
                        transform.y = Number(transformObject.y);
                        transform.skewX = Number(transformObject.skX) * utils.ConstValues.ANGLE_TO_RADIAN;
                        transform.skewY = Number(transformObject.skY) * utils.ConstValues.ANGLE_TO_RADIAN;
                        transform.scaleX = Number(transformObject.scX);
                        transform.scaleY = Number(transformObject.scY);
                    }
                    if (pivot) {
                        pivot.x = Number(transformObject.pX);
                        pivot.y = Number(transformObject.pY);
                    }
                }
            };
            return DataParser;
    ...
    (function (factorys) {
        var BaseFactory = (function (_super) {
            ...
            BaseFactory.prototype.buildArmature = function (display, armatureName, animationName, skeletonName, textureAtlasName, skinName) {
                if (skeletonName) {
                ...
                var armature = this._generateArmature(display);
                    ...
                    while (i--) {
                        ...
                        switch (displayData.type) {
                            case objects.DisplayData.ARMATURE:
                                childArmature = this.buildArmature(null,displayData.name, null, this._currentDataName, this._currentTextureAtlasName, null);
                                if (childArmature) {
    ...
    var Armature = (function (_super) {
        ...
        Armature.prototype.setDisplay = function (display) {
            this._display = display;
        };
        ...
        Armature.prototype.updateSlotsZOrder = function () {
            ...
            while (i--) {
                slot = this._slotList[i];
                if (slot._isDisplayOnStage) {
                    slot._displayBridge.addDisplay(this._display, slot.getZOrder());
                }
            }
